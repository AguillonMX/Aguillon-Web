...
carruselConteiner.addEventListener("mouseenter", () =>
...
// Función para copiar el código de creador
function copiarCodigo() {
  const codigo = document.getElementById('codigoValor');
  const texto = codigo.innerText;
  
  navigator.clipboard.writeText(texto).then(function() {
    const mensaje = document.getElementById('copiarMensaje');
  ...
  contactForm.addEventListener('submit', function(e) {
    ...
    .then(response => {
      if (response.ok) {
        ...
        //Opcional recargar el formulario despues de 5 segundos
        setTimeout(() => {
